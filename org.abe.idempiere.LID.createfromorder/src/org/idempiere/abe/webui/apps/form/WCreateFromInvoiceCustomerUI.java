...
public class WCreateFromInvoiceCustomerUI extends CreateFromInvoice implements EventListener<Event>, ValueChangeListener
{
	private WCreateFromWindow window;
	
	public WCreateFromInvoiceCustomerUI(GridTab tab) 
	{
		super(tab);
	...
	/**	Logger			*/
	private static final CLogger log = CLogger.getCLogger(WCreateFromInvoiceCustomerUI.class);
...
//	protected Listbox shipmentField = ListboxFactory.newDropdownListbox();
	protected BandboxSearch shipmentField = new BandboxSearch();
	...
	public void onEvent(Event e) throws Exception
	{
		...
		//  Shipment
		else if (e.getTarget().equals(shipmentField.getListbox()))
		{
			...
			shipmentField.setRawValue(pp.getName());
			shipmentField.close();
...
//			orderField.addItem(knp);
	 ...
	 * @param C_BPartner_ID
	 */
	private void initBPShipmentDetails(int C_BPartner_ID,String filter)
	{
		if (log.isLoggable(Level.CONFIG)) log.config("C_BPartner_ID" + C_BPartner_ID);
	...
	private void initBPShipmentDetails(int C_BPartner_ID) {
		initBPShipmentDetails(C_BPartner_ID,null);
	}
